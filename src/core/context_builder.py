...
def compose_context(app, cfg, budget_chars: int = 1800) -> str:
    ...
    if cfg.mt5_enabled:
        try:
            ...
            logging.exception("Failed to build MT5 context. Will proceed without it.")
            mt5full = None
    ...
    try:
        ...
        if len(text) > budget_chars:
             logging.warning(f"Context still too long ({len(text)} chars) after slimming, but returning full valid JSON to avoid corruption.")
        
        return text

    except Exception as e:
        logging.exception(f"Error during context slimming: {e}")
