"""
Định nghĩa RunConfig, đọc/ghi cấu hình, các hằng số cấu hình
"""
...
class RunConfig:
    """
    Lớp RunConfig đại diện cho cấu hình chạy của ứng dụng.
    """
    folder: str
    delete_after: bool
    max_files: int
...
# Hàm đọc/ghi cấu hình workspace ví dụ:
def save_workspace(data: dict, path: Path):
...
def delete_workspace(path: Path):
    if path.exists():
        path.unlink()